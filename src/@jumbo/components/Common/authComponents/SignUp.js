...
const SignUp = ({method = CurrentAuthMethod, variant = 'default', wrapperVariant = 'default'}) => {
    ...
    const onSubmit = () => {
        if (name.trim() === '' || password.trim() === '') {
            ...
            // TODO
            // game api - /user/create
            /*
            gameService.post('user/create', {username: name, nickname: nickname})
                .then(res => {
                    if (res.status === 200) {
                        let id = res.data.id;
                        dispatch(AuthMethods[method].onRegister({id, name, nickname, password}));
                    } else {
                        NotificationManager.err(res.errors.username, 'Error');
                    }
                })
                .catch(function (err) {
                    NotificationManager.err(err.message, 'Error');
                });
            */
            axios.post('user/create', {username: name, nickname: nickname})
                .then(res => {
                    if (res.status === 200) {
                        let id = res.data.id;
                        dispatch(AuthMethods[method].onRegister({id, name, nickname, password}));
    ...
    return (
        <AuthWrapper variant={wrapperVariant}>
            ...
            <Box className={classes.authContent}>
                ...
                <Box mb={3}>{dispatch(AuthMethods[method].getSocialMediaIcons())}</Box>
